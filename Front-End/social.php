<title>សង្គម | Magazine Online Template</title>
<!-- ============== HEADER ============== -->
<?php
    include("navbar.php");
?>
...
<section id="banner" class="mt-20">
    <?php
        ...
        $sql_select="SELECT * FROM tbl_social_banner ORDER BY id DESC LIMIT 1";
...
<div
    ...
    <div class="md:w-3/4 flex flex-col">
        <h1 class="text-2xl md:text-4xl font-bold text-blue-800">សង្គម</h1>
        <div style="border:2px dashed #337ab7;margin:5px 0 10px 0px"></div>
        <div style="border:1px solid #337ab7;margin:5px 0 10px 0px"></div>

        <?php
            $sql_select = "SELECT * FROM tbl_news WHERE news_type='សង្គម' ORDER by id DESC";
            $result = $con->query($sql_select);
